...
const TFunction MainUserBootloader  = (TFunction)(0x6000ul);
...
static void xmodem_receive(void)
{
  ...
  x_packet_number = 1u;
  x_actual_flash_address = FLASH_OTAAPP_START_ADDR;//FLASH_APP_START_ADDR;//FLASH_OTAAPP_START_ADDR;
...
static xmodem_status xmodem_handle_packet(uint8_t header)
{
  ...
  if (FLASH_OK != Xmoden_FlashWrite(x_actual_flash_address, (uint8_t*)&received_data[X_PACKET_DATA_INDEX], (uint16_t)size))
  {
    /* Flashing error. */
    status |= X_ERROR_FLASH;
  }
